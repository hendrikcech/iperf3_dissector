...
do
    ...
    iperf_loss_E = ProtoExpert.new("iperf.packet_loss", "Packet loss (or reordering) detected",
                                expert.group.PROTOCOL, expert.severity.WARN)
    iperf_ooo_E = ProtoExpert.new("iperf.out_of_order", "Packet arrived out-of-order",
                                expert.group.PROTOCOL, expert.severity.WARN)
    iperf_proto.experts = { iperf_loss_E, iperf_ooo_E }

    local port_seq = {}
    local port_pkts_ooo = {}
    local port_pkts_after_gap = {}
    ...
    function iperf_proto.dissector(buffer,pinfo,tree)
        ...
        local iperf_seq_str = tostring(iperf_seq)
        if not pinfo.visited then
            if not port_seq[port_str] then
                port_seq[port_str] = iperf_seq
                port_pkts_ooo[port_str] = {}
                port_pkts_after_gap[port_str] = {}
            else
                local expected = port_seq[port_str] + 1;
                if iperf_seq > expected then
                    -- print("Loss: packet", pinfo.number, "seq", iperf_seq)
                    port_pkts_after_gap[port_str][iperf_seq_str] = 1
                elseif iperf_seq < expected then
                    -- print("OOO: packet", pinfo.number, "seq", iperf_seq)
                    port_pkts_ooo[port_str][iperf_seq_str] = 1
                end
                port_seq[port_str] = iperf_seq
            end
        end

        if port_pkts_after_gap[port_str][iperf_seq_str] then
            subtree:add_proto_expert_info(iperf_loss_E)
        end
        if port_pkts_ooo[port_str][iperf_seq_str] then
            subtree:add_proto_expert_info(iperf_ooo_E)
        end
